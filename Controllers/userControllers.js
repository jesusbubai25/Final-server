...
exports.verifyOtpHandler = async (req, res) => {
  ...
  try {
    ...
    await sendEmail(options)
    return res.status(200).cookie("auth_otp", null, { expires: new Date(Date.now()), httpOnly: true ,sameSite:"none",secure:false}).json({ message: "Otp verified sucessfully! and password sent to your email", sucess: true })
